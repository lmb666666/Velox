...
export default function App() {
  ...
  const view = viewHistory
    ? { phase: 'done' as ResultPhase, frames: viewHistory.detail.frames, statusLines: [], result: { summary: viewHistory.detail.summary, frames: viewHistory.detail.frames, finished: viewHistory.detail.finished, reason: viewHistory.detail.reason }, error: viewHistory.detail.error, taskId: viewHistory.id }
    : task
  ...
  return (
    <div className="min-h-screen">
      ...
      <div aria-hidden className="pointer-events-none fixed inset-x-0 top-0 z-0 h-64 bg-[radial-gradient(ellipse_60%_60%_at_50%_-10%,hsl(187_86%_53%/0.12),transparent)]" />

      <header className="sticky top-0 z-40 border-b border-border/60 bg-background/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
          <BrandLockup loading={running} />
          <div className="flex items-center gap-2">
            <Badge variant="secondary" className="num hidden sm:inline-flex">
              {nodesData ? `${nodesData.total} 监测点` : '…'}
            </Badge>
            <ThemeToggle />
          </div>
        </div>
