...
import { Activity, ArrowRightLeft, ChevronDown, FileSearch, Globe, Layers, Network, Play, Route, Settings2 } from 'lucide-react';
  ...
  function submit(e: React.FormEvent) {
    ...
    onSubmit({
      ...
      ...(mode === 'http' && {
        ...
        cookie: cookie.trim(),
        httpUa: httpUa.trim(),
        resolveTo: resolveTo.trim(),
        redirects: Number.parseInt(redirects, 10) || 5,
      }),
    });
  }
                ...
                <div className="mt-2 grid gap-3 rounded-lg border bg-muted/30 p-3 sm:grid-cols-2">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      placeholder="例：key=123;page=123"
                      className="num h-8 text-xs"
                    />
                  </div>
                  <div className="space-y-1.5 sm:col-span-2">
                    ...
                    <Input
                      id="http-ua"
                      value={httpUa}
                      onChange={(e) => setHttpUa(e.target.value)}
                      placeholder="留空使用默认（监测节点请求目标时携带）"
                      className="num h-8 text-xs"
                    />
                  </div>
                </div>
